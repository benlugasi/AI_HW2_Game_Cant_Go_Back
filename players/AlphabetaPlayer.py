...
class Player(AbstractPlayer):
    ...
    def make_move(self, time_limit, players_score):
        ...
        last_valid_move = self.alphaBeta.search(state=cur_state, depth=depth, maximizing_player=1, time_limit=time_limit)
        ...
        while depth < len(self.board)*len(self.board[0]):  # total number of possible steps on the board
            ...
            move = self.alphaBeta.search(state=cur_state, depth=depth, maximizing_player=1, time_limit=time_limit-time_elapsed)
            if move is not None:
    ...
    #def goal(self, state):
        ...
        #return AbstractPlayer.goal(self, state)
    ...
    #def succ(self, state, maximizing_player):  # ->List(directions)
        ...
        # return succ_states
        #return AbstractPlayer.succ(self, state, maximizing_player)
    ...
    #     return Player.PlayerState(self.board, new_player_to_move, new_fruit_life, fruit_taken, penalty_taken)
        #return AbstractPlayer.perform_move(self, state=state, d=d, maximizing_player=maximizing_player)
    ...
    #     self.score[state.playerToMove-1] = self.score[state.playerToMove-1] - next_state.fruit_taken + next_state.penalty_taken   # reverts score
        # AbstractPlayer.revert_move(self, state=state, next_state=next_state)

    # def utility(self, state):
    #     assert (self.goal(state))
    #     return self.score[0] - self.score[1]
        # AbstractPlayer.utility(self, state=state)
    ...
    #     return (self.score[0] + my_potential_score) - (rival_potential_score + self.score[1])
        # AbstractPlayer.heuristic_function(self, state=state)
