...
class Player(AbstractPlayer):
    ...
    def make_move(self, time_limit, players_score):
        ...
        if last_valid_move is None:
            for d in self.directions:
                ...
                if 0 <= i < len(self.board) and 0 <= j < len(self.board[0]) and self.board[new_pos] not in [-1, 1, 2]:
                    ...
                    print(f"global_time_alpha_beta random depth: {depth} time frame: {time_frame}")
                    return d
        else:
        ...
        assert utils.count_val(self.board, 1) == 1
        self.pos = new_pos
        print(f"Global_time_alpha_beta depth: {depth} time frame: {time_frame}")
    ...
    # ########## helper functions for the GlobalTimeABPlayer algorithm ##########
